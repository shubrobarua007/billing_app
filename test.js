...
const invoicesFolder = path.join(__dirname, process.env.INVOICE_FOLDER );
...
app.post('/generate-pdf', async (req, res) => {
    try {
      ...
      //shop logo
        const shopLogoPath = path.resolve(__dirname, process.env.LOGO_PATH);
        let logoBase64 = "";
    ...
    const convertToWords = (amount)=>{
        ...
        const dirhamsWord = toWords(dirhams).toUpperCase();
        

        if(fils > 0){
            const filsWord = toWords(fils).toUpperCase();
            return `${dirhamsWord} DIRHAMS AND ${filsWord} FILS ONLY`; 
        }else{
            return `${dirhamsWord} DIRHAMS ONLY`;
        }
    };
